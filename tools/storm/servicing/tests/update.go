...
func innerUpdateLoop(tc storm.TestCase, testConfig stormsvcconfig.TestConfig, vmConfig stormvmconfig.AllVMConfig, rollback bool) error {
	...
	for i := 1; i <= loopCount; i++ {
		...
		// SSH reverse tunnels are needed only during staging (for COSI download).
		// Use a per-iteration context so proxies are cleanly stopped before
		// finalize triggers a VM reboot, preventing goroutine accumulation.
		...
		defer cleanupProxies() // safety net for error returns
...
func checkSerialLogForDracutIssues(serialLogPath string, iteration int) {
	...
	dracutPatterns := []struct {
		...
		{"Entering emergency mode", "initramfs entered emergency mode", true},
