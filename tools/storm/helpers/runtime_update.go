...
type RuntimeUpdateHelper struct {
	args struct {
		...
		StageRuntimeUpdate            bool     `short:"s" help:"Controls whether A/B update should be staged."`
		FinalizeRuntimeUpdate         bool     `short:"f" help:"Controls whether A/B update should be finalized."`
...
func (h *RuntimeUpdateHelper) updateHostConfig(tc storm.TestCase) error {
	...
	// Delete any scripts as this will prevent a runtime update from running
	...
	for i := 1; ; i++ {
		...
		if err != nil {
			if err, ok := err.(*ssh.ExitMissingError); ok && !strings.Contains(out.Stderr, "Rebooting system") {
				// The connection closed without an exit code, and the output does *not*
				// contain "Rebooting system".
				logrus.Infof("Runtime update was successful")
				break
			} else {
