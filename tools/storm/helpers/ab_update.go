...
func (h *AbUpdateHelper) checkDiagnostics(tc storm.TestCase) error {
	...
	// Run trident diagnostics
	bundlePath := "/tmp/bundle"
	logrus.Infof("Running trident diagnostics with output file: %s", bundlePath)
	out, err := stormtrident.InvokeTrident(h.args.Env, h.client, h.args.EnvVars, fmt.Sprintf("diagnose --output %s", bundlePath))
	if err != nil {
	...
	if err := stormsshclient.CopyRemoteFileToLocal(h.client, bundlePath, localBundlePath); err != nil {
		return err
	}
	defer os.Remove(localBundlePath)

	if err := h.verifyDiagnosticBundleContents(localBundlePath); err != nil {
		return err
	}
...
func (h *AbUpdateHelper) verifyDiagnosticBundleContents(bundlePath string) error {
