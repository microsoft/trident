...
// scenario's progress through the AKS agent's stage/finalize/commit state
...
// expectValidateConnection runs "trident-aks-agent --validate-connection
// <mode>" on the VM and asserts its exit status matches wantSuccess. All
// configuration is via TRIDENT_AKS_AGENT_* environment variables (see
// crates/trident-aks-agent/src/config.rs) - there is no config file - so
...
// defaults; after prepareVmForAksAgent delivers it, kubernetes succeeds
...
func expectValidateConnection(cfg stormvmconfig.VMConfig, vmIP, mode string, wantSuccess bool, envVars map[string]string) error {
	...
	command := fmt.Sprintf("%s trident-aks-agent --validate-connection %s", prefix.String(), mode)
...
func RunABUpdate(testConfig stormaksconfig.TestConfig, vmConfig stormvmconfig.AllVMConfig) error {
	...
	// runtime update; it is not enough for the aks-agent to merely reach the
	...
	// Before the fake kubeconfig has been delivered to the VM, trident-aks-agent
	...
	// Once prepareVmForAksAgent has delivered the fake kubeconfig, kubernetes
	// now succeeds (its own `server:` field points at the fake apiserver);
	// tridentd is unaffected either way. trident-aks-agent never gets a
	// config file at all (see prepareVmForAksAgent's doc comment) - Nebraska
	...
	// (for this one-off diagnostic check) a TRIDENT_AKS_AGENT_NEBRASKA_*
	...
	if err := expectValidateConnection(vmConfig.VMConfig, vmIP, "nebraska", false, nil); err != nil {
		return fmt.Errorf("post-config validate-connection check failed (trident-aks-agent has no static Nebraska config): %w", err)
	}
	...
	// exact TRIDENT_AKS_AGENT_NEBRASKA_* values RunABUpdate is about to send
	...
	nebraskaEnv := map[string]string{
		"TRIDENT_AKS_AGENT_NEBRASKA_ENDPOINT": nebraskaServer,
		"TRIDENT_AKS_AGENT_NEBRASKA_APP_ID":   nebraskaAppID,
		"TRIDENT_AKS_AGENT_NEBRASKA_TRACK":    nebraskaTrack,
	}
	...
	wrongNebraskaEnv := map[string]string{
		"TRIDENT_AKS_AGENT_NEBRASKA_ENDPOINT": fmt.Sprintf("http://%s:%d", testConfig.HostEndpointIP, testConfig.NebraskaPort+1),
		"TRIDENT_AKS_AGENT_NEBRASKA_APP_ID":   nebraskaAppID,
		"TRIDENT_AKS_AGENT_NEBRASKA_TRACK":    nebraskaTrack,
	}
	...
	if err != nil {
		...
		return fmt.Errorf("AKS agent scenario failed (stage/finalize): %w", err)
	}
	if !report.Passed {
		collectAksArtifactsBestEffort(vmConfig.VMConfig, vmIP, testConfig.OutputPath)
		return fmt.Errorf("AKS agent scenario failed (stage/finalize): %+v", report)
	}

	// Finalize triggers a real "systemctl reboot" from trident-aks-agent
	...
	// kubeconfig prepareVmForAksAgent delivered before staging carry over
	// unchanged to the newly-activated root. trident-aks-agent.service is
	...
	if err != nil {
		collectAksArtifactsBestEffort(vmConfig.VMConfig, vmIP, testConfig.OutputPath)
		return fmt.Errorf("AKS agent scenario failed (post-reboot commit): %w", err)
	}
	if !finalReport.Passed {
		collectAksArtifactsBestEffort(vmConfig.VMConfig, vmIP, testConfig.OutputPath)
		return fmt.Errorf("AKS agent scenario failed (post-reboot commit): %+v", finalReport)
	}
	...
	// The node annotation only proves the AKS-agent-facing rollout API
	// reported success; it says nothing about whether trident-aks-agent
	...
	if err := nebraska.ValidateStatusHistory(stormproxies.ExpectedUpdateStatusSequence); err != nil {
		collectAksArtifactsBestEffort(vmConfig.VMConfig, vmIP, testConfig.OutputPath)
		return fmt.Errorf("AKS agent scenario failed Nebraska status validation: %w", err)
	}

	return collectAksArtifacts(vmConfig.VMConfig, vmIP, testConfig.OutputPath)
}

// collectAksArtifactsBestEffort collects the same diagnostic artifacts as
// collectAksArtifacts, but on a failure path where the harness is about to
...
// so nothing ever calls collectAksArtifacts and the post-reboot journal
// (trident-aks-agent.log / tridentd.log) that would explain the failure is
...
func collectAksArtifactsBestEffort(cfg stormvmconfig.VMConfig, vmIP string, outputPath string) {
	if err := collectAksArtifacts(cfg, vmIP, outputPath); err != nil {
		logrus.Warnf("best-effort artifact collection after test failure also failed: %v", err)
	}
}

func prepareVmForAksAgent(cfg stormvmconfig.VMConfig, vmIP string, testConfig stormaksconfig.TestConfig) error {
	// trident-aks-agent needs no config file at all for this scenario:
	...
	//     (baseimg-aks-agent.yaml), so the agent's own hostname-derived
	...
	// trident-aks-agent.conf is simply never written to the VM.
...
`, testConfig.HostEndpointIP, testConfig.APIServerPort)
	...
	localKubeconfigFile, err := os.CreateTemp("", "trident-aks-agent-kubeconfig-*.yaml")
	if err != nil {
	...
	// trident-aks-agent.service. Each RunX test case starts its own fresh
	...
	command := strings.Join([]string{
		...
		"sudo systemctl enable trident-aks-agent.service",
		"sudo systemctl restart trident-aks-agent.service",
		...
		// Lock in the design invariant: trident-aks-agent.conf is never
		...
		// fields.
		"! sudo test -e /etc/trident/trident-aks-agent.conf",
	}, " && ")
	if _, err := stormssh.SshCommandCombinedOutput(cfg, vmIP, command); err != nil {
		return fmt.Errorf("failed to prepare VM for AKS agent: %w", err)
	}
	...
	for _, svc := range []string{"tridentd.service", "trident-aks-agent.service"} {
...
func waitForVmRebootAndSshBack(vmConfig stormvmconfig.AllVMConfig, vmIP string, testConfig stormaksconfig.TestConfig) error {
	wentDown := false
	...
	// real SSH commands (e.g. prepareVmForAksAgent) don't race a
	// still-settling boot.
...
func collectAksArtifacts(cfg stormvmconfig.VMConfig, vmIP string, outputPath string) error {
	if outputPath == "" {
		return nil
	}
	cmds := []string{
		"sudo journalctl --no-pager -u trident-aks-agent.service > /tmp/trident-aks-agent.log && sudo chmod 644 /tmp/trident-aks-agent.log",
	...
	for _, remote := range []string{"/tmp/trident-aks-agent.log", "/tmp/tridentd.log"} {
