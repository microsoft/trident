...
type TestConfig struct {
	...
	NodeName              string `help:"Node name served by the fake apiserver; must match the VM image's hostname (Image Customizer 'hostname' setting in baseimg-aks-agent.yaml), since trident-aks-agent's [kubernetes].node_name defaults to the node's own real hostname" default:"trident-aks-agent-testimg"`
