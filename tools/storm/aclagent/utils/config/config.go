...
type TestConfig struct {
	...
	ImagePath             string `help:"Path to a real OS update image (e.g. a .cosi file) to serve to tridentd during staging; when set, this takes precedence over NebraskaCodebase/NebraskaPackageName/NebraskaSHA384. When empty, ArtifactsDir must contain exactly one *.cosi file, which is used."`
