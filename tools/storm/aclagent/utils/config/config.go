...
type TestConfig struct {
	...
	ImagePath             string `help:"Path to a real OS update image (e.g. a .cosi file) to serve to tridentd during staging; when set, this takes precedence over NebraskaCodebase/NebraskaPackageName/NebraskaSHA384 and CosiPattern. When empty, ArtifactsDir is searched recursively using CosiPattern, which must match exactly one file." default:""`
	CosiPattern           string `help:"Regex pattern used to find the update .cosi image under ArtifactsDir when ImagePath is not set; narrow this to the scenario's own update image name to avoid colliding with other scenarios' .cosi files in a shared artifacts directory" default:".*\\.cosi$"`
