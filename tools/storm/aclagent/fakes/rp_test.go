...
func TestScenarioRunnerObservesExpectedState(t *testing.T) {
	...
	go func() {
		...
		store.PatchLabels(map[string]string{StateLabel: "staged", ObservedRequestIDLabel: "R1"})
	}()

	runner := &RPClient{APIServerURL: server.URL, NodeName: "node-a"}
	scenario := &Scenario{Steps: []ScenarioStep{{Patch: &PatchStep{Request: "stage", RequestID: "R1", TargetOSImageVersion: "202507.28.0"}}, {Expect: &ExpectStep{State: "staged", ObservedRequestID: "R1", Timeout: 2 * time.Second}}}}
