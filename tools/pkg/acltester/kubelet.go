...
const RebootStateAnnotation = "trident-acl-agent-tester/reboot-state"
	...
	for {
		...
		case <-ticker.C:
			if _, err := os.Stat(k.MarkerFile); err == nil {
				...
				if err := patchStringMap(ctx, k.client(), k.nodeURL(), "annotations", map[string]string{
					RebootStateAnnotation: "not-ready",
				}); err != nil {
					return err
				}
				select {
