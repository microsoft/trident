## NOTE:
## These tests are currently disabled as they don't work with the new fastVM
## implementation because it is not deployed by Trident.
##
## They are being kept here for reference and can be re-enabled if trident is
## copied into the machine.
##
## All these tests, expect for test_trident_start_network, are currently
## covered by some other test, either FT or E2E. SO it is possible that we will
## just fully remove this file.
...
#     for id in host_status["partitionPaths"]:
#         host_status["partitionPaths"][id] = placeholder
#     host_status["diskUuids"] = {placeholder: placeholder}
#     with open(
