...
pub async fn run_omaha_only(config: &config::AgentConfig) -> Result<(), anyhow::Error> {
    ...
    match outcome {
        ...
        CheckOutcome::UpdateAvailable(offer) => {
            ...
            // The nebraska client module does not parse the package hash out
            // of the Omaha manifest (see nebraska::wire::Package) - integrity
            // of the downloaded image is verified by Trident itself via the
            // image's own COSI metadata, so no hash is passed here.
            client
                .update(&offer.package_url, None, combined_timeout)
                .await?;
            Ok(())
        }
    }
