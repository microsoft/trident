...
| 8 | ACL-agent storm reboot behavior | Decided for test coverage | The storm scenario keeps a reboot shim instead of performing a full guest reboot. This is less realistic than the servicing scenario's real reboot path, but keeps reverse SSH tunnels and the in-process fake apiserver / Nebraska endpoints stable while still exercising the finalize-triggered reboot intent and the Ready flip semantics through the fake kubelet helper. |
...
- **Timeout specifics.** Per-phase timeouts have a *shape* (COSI-download-sized
  ...
  in `config.rs`) are placeholders pending real data from the storm `aclagent`
  scenario runs. Open: whether these should be static constants or configurable
  per-SKU/pool (large COSIs on slow storage vs. fast NVMe-backed SKUs may need
  different bounds).
...
aks-rp/kubelet/Nebraska. The storm `aclagent` scenario expects a test image to
pre-seed this file with `localhost:<port>` endpoints that are backed by reverse
SSH forwarding into in-process fake services running on the storm runner.
