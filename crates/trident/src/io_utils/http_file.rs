...
use crate::io_utils::partial_reader::PartialReader;
#[cfg(feature = "dangerous-options")]
...
impl HttpFile {
    ...
    fn new_inner(
        ...
        let response = Self::retriable_request_sender(request_sender, timeout)?
            .ok_or_else(|| IoError::new(IoErrorKind::Other, "No response from server"))?;
    ...
    /// Returns a PartialReader that can handle partial content delivery.
    pub fn reader(&self, start: Option<u64>, end: Option<u64>) -> IoResult<PartialReader> {
        PartialReader::new(
            self.client.clone(),
            self.url.clone(),
            self.token.clone(),
    ...
    /// Performs an HTTP request and retries it for up to `timeout` if it fails.
    /// The HTTP request is created and invoked by `request_sender`, a closure that
    /// returns a `reqwest::Result<Option<Response>>`. The closure can return `Ok(None)`
    /// to indicate there's no more data to fetch.
    ///
    /// This function handles partial content delivery - if the server returns less data
    /// than requested, it will accept what's available and allow the caller to retry
    /// for the remaining bytes later.
    ///
    /// If the request is successful, it returns the response. If the request fails
    /// after all retries, it returns an IO error.
    pub fn retriable_request_sender<F>(
        request_sender: F,
        timeout: Duration,
    ) -> IoResult<Option<Response>>
    where
        F: Fn() -> reqwest::Result<Option<Response>>,
    {
        let mut retry = 0;
        let now = Instant::now();
        ...
        loop {
            ...
            match request_sender() {
                Ok(None) => {
                    // Caller indicates no more data to fetch
                    return Ok(None);
                }
                Ok(Some(response)) => {
    ...
    pub fn section_reader(&self, section_offset: u64, size: u64) -> IoResult<PartialReader> {
        ...
        self.reader(Some(section_offset), Some(end))
    }
}
...
impl Read for HttpFile {
    fn read(&mut self, buf: &mut [u8]) -> IoResult<usize> {
        let mut reader = self.section_reader(self.position, buf.len() as u64)?;
    ...
    fn read_to_end(&mut self, buf: &mut Vec<u8>) -> IoResult<usize> {
        let mut reader = self.reader(Some(self.position), None)?;
        let res = reader.read_to_end(buf)?;
        self.position += res as u64;
        Ok(res)
    }
}
    ...
    fn test_retriable_request_sender_initial_failure() {
        ...
        let request_sender = || {
            ...
            if closure_tries.load(Ordering::SeqCst) < 2 {
                let client = Client::new();
                return client.get("").send().map(Some);
            }
            let client = Client::new();
            client.get(&request_url).send().map(Some)
        };
