...
pub(crate) fn update(
    ...
    if state.host_status().servicing_state == ServicingState::AbUpdateStaged {
        // Need to re-set the Host Status in case an A/B update has been previously staged
    ...
    if state.host_status().servicing_state == ServicingState::RuntimeUpdateStaged {
        // Need to re-set the Host Status in case a runtime update has been previously staged
