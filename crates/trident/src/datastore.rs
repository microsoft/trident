...
use trident_api::{
    ...
    status::{decode_host_status, HostStatus, ServicingState, TridentVersion},
};
...
impl DataStore {
    ...
    /// Returns this datastore's installation ID, minting one as a
    /// one-time migration if it is missing. Single source of truth for
    /// every caller that attaches an installation ID to the shared
    /// `TraceStream` on datastore open, so the same migration logic isn't
    /// hand-copied (and drifting) across `Trident::new`, the CLI's
    /// pre-`command_start` pre-warm, and the daemon's per-request
    /// backstop.
    ///
    /// A datastore can be genuinely provisioned (`servicing_state !=
    ...
    /// host actually completes an install.
    pub fn installation_id_or_migrate(&mut self) -> Result<Option<Uuid>, TridentError> {
    ...
    /// Single source of truth for "may this command/request create a
    /// brand-new datastore, and therefore eager-mint (not just migrate) an
    /// installation ID via [`Self::create_installation_id`]?", shared by
    /// the CLI's pre-`command_start` pre-warm (`main.rs`) and the daemon's
    /// per-request backstop (`server/tridentserver/mod.rs`'s
    /// `refresh_installation_id`), so both surfaces answer this question
    /// identically instead of each re-deriving it independently.
    ///
    ...
    /// create one. Finalize-only names (`install_finalize`,
    /// `update_finalize`) cannot stage anything themselves -- they require
    /// an existing staged state, so letting them create a datastore/mint
    /// an installation ID would leave one behind even when the request is
    /// then rejected (e.g. a premature `install_finalize` on an
    /// unprovisioned host) for having no staged operation to finalize.
...
mod tests {
    ...
    #[test]
    /// Regression test for DR-010/DR-011: `may_initialize_datastore_for_command`
    /// is the single shared classifier answering "may this command/request
    /// create a brand-new datastore and eager-mint an installation ID?" for
    /// both the CLI (`main.rs`) and the daemon
    /// (`server/tridentserver/mod.rs`'s `refresh_installation_id`). Stage
    /// names (and `stream_disk`) may; finalize-only names must not, since a
    /// finalize-only request cannot itself stage anything, and letting it
    /// create a datastore would leave one behind even when the request is
    /// then rejected for having nothing staged to finalize. Also covers
    /// (DR-024) the `_noop` command-name variants (`command_name` in
    /// `main.rs` emits these when neither stage nor finalize is
    /// requested): they must stay denied even though they aren't
    /// explicitly excluded by name, so a future refactor that widens the
    /// allow-list can't silently start creating datastores for no-op
    /// invocations without this test catching it.
    ...
    #[test]
    /// Regression test: `new_servicing_id` always mints a *fresh* ID on
    /// every call, unlike `create_installation_id`'s get-or-create
    /// semantics. This is what lets a retried clean install (e.g. a
    /// previous attempt's `clean_install_safety_check` or staging failed,
    /// leaving `ServicingState::NotProvisioned`) get its own distinct
    /// servicing ID rather than reusing a stale one from the failed
    /// attempt.
    fn test_new_servicing_id_mints_a_fresh_id_each_call() {
        let temp_dir = tempfile::tempdir().unwrap();
        let datastore_path = temp_dir.path().join("db.sqlite");
        ...
        assert_ne!(
            first, second,
            "new_servicing_id should mint a fresh ID on every call, not reuse the previous one"
        );
