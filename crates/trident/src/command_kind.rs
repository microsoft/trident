...
enum Family {
    ...
    Rollback { check: bool, phase: Phase },
    Commit,
    RebuildRaid,
    StreamDisk,
...
mod tests {
    ...
    fn grpc_client_strips_prefix_and_matches_cli_naming() {
        ...
        assert_eq!(
            client(&["rebuild-raid"]).kind().as_str(),
            "rebuild_raid"
        );
        assert_eq!(
    ...
    fn is_servicing_excludes_check_rollback_and_read_only_commands() {
        ...
        assert!(cli(&["rollback", "--allowed-operations", "stage"])
            .kind()
            .is_servicing());
        assert!(!cli(&["rollback", "--check", "--allowed-operations", "stage"])
            .kind()
            .is_servicing());
    ...
    fn owns_local_metrics_file_matches_truncate_set() {
        ...
        // service one.
        assert!(cli(&["install", "--allowed-operations", "stage,finalize"])
            .owns_local_metrics_file());
        assert!(cli(&["update", "--allowed-operations", "stage,finalize"])
            .owns_local_metrics_file());
        assert!(cli(&["commit"]).owns_local_metrics_file());
        assert!(cli(&["rebuild-raid"]).owns_local_metrics_file());
        assert!(cli(&["rollback", "--allowed-operations", "stage,finalize"])
            .owns_local_metrics_file());
        ...
        assert!(!cli(&["rollback", "--check", "--allowed-operations", "stage"])
            .owns_local_metrics_file());
        assert!(!cli(&["grpc-client", "install", "--allowed-operations", "stage"])
            .owns_local_metrics_file());
    ...
    fn phase_strip_round_trips_every_suffix() {
        ...
        assert_eq!(Phase::strip("install_finalize"), ("install", Phase::Finalize));
    ...
    fn daemon_constructors_match_expected_names_and_classification() {
        ...
        assert_eq!(CommandKind::rollback_finalize().as_str(), "rollback_finalize");
