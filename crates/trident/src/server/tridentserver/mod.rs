...
use std::{panic, path::Path, sync::Arc, time::SystemTime};
...
impl TridentServer {
    ...
    /// here.
    fn refresh_ids(tracestream: &TraceStream, datastore_path: &Path) {
        tracestream.attach_ids_if_present(datastore_path, false);
    }
            ...
            + 'static,
    {
        ...
        // request's shared Tokio worker thread for that long before the
        // real servicing work even reaches `spawn_blocking`. The closure
        // itself runs entirely inside `spawn_servicing_task`'s
        // `spawn_blocking` (see there), which has its own dedicated OS
        // thread, so the same I/O there can't block anything else.
        // Captures owned clones (not `self`) since the closure must be
        // `'static`.
        let tracestream_for_ids = self.tracestream.clone();
        let datastore_path_for_ids = self.agent_config.datastore_path().to_path_buf();
        ...
        tokio::spawn(async move {
            ...
            if let Some(ref err) = completed_message.error {
                error!(
                    "Servicing request '{}' failed: {}",
                    name_for_async, err.message
                );
            } else {
                info!(
                    "Servicing request '{}' completed successfully",
                    name_for_async
                );
            }
