...
pub struct AbVolumePair {
    ...
    ///
    /// This is a user-defined string that links the A/B volume pair to the `filesystems` config in
    /// the Host Configuration. The identifier needs to be unique across devices of all types, not
    /// just A/B volume pairs.
