...
impl IdSource {
    pub(super) fn produce_id(&self) -> Result<String, AgentCoreError> {
        Ok(match self {
            IdSource::MachineIdHashed => MachineId::read()
                .map_err(|err| AgentCoreError::MachineIdRead(err.to_string()))?
                .hashed_uuid()
                .to_string(),
            IdSource::MachineIdRaw => MachineId::read()
                ...
                .as_string(),
            IdSource::Hostname => {
                hostname::read().map_err(|err| AgentCoreError::HostnameRead(err.to_string()))?
            }
        })
    }
}
