...
pub(crate) const PCRLOCK_DIR: &str = "/var/lib/pcrlock.d";

/// Name of the pcrlock policy JSON file that represents the TPM 2.0 access policy that has been
/// generated by `systemd-pcrlock`. This file is placed into the directory adjacent to the
/// datastore.
...
/// expected.
fn generate_tpm2_access_policy(
    pcrs: BitFlags<Pcr>,
    pcrlock_policy_path: &Path,
) -> Result<(), Error> {
    debug!(
        ...
        .unstructured("Failed to determine if running in container")?
    {
        ...
        let host_pcrlock_json_path = path::join_relative(host_root, pcrlock_policy_path);
        if host_pcrlock_json_path.exists() {
            ...
            fs::copy(host_pcrlock_json_path, pcrlock_policy_path)
    ...
    make_policy(pcrs, pcrlock_policy_path)
        .context("Failed to run 'systemd-pcrlock make-policy' command")?;

    // Log pcrlock policy JSON contents
    let pcrlock_policy = fs::read_to_string(pcrlock_policy_path).context(format!(
        ...
        .unstructured("Failed to determine if running in container")?
    {
        ...
        fs::copy(pcrlock_policy_path, host_pcrlock_json_path)
...
fn make_policy(pcrs: BitFlags<Pcr>, pcrlock_policy_path: &Path) -> Result<(), Error> {
    debug!(
    ...
    // Use the dependency system to execute the command and capture both stdout and stderr
    let (stdout_str, stderr_str) = Dependency::SystemdPcrlock
        .cmd()
        .arg("make-policy")
        .arg(to_pcr_arg(pcrs))
...
pub fn remove_policy(pcrlock_policy_path: &Path) -> Result<(), Error> {
    // Remove the pcrlock policy
    let mut pcrlock_policy = vec![PathBuf::from(pcrlock_policy_path)];
