...
"#;
    ...
    fn test_extract_cmdline_from_bls_entries_picks_newest_non_rescue() {
        ...
        mk("abc123-0-rescue.conf", "root=UUID=rescue rescue selinux=0");
        mk(
            "abc123-6.6.10-1.azl4.conf",
            "root=UUID=old selinux=1 enforcing=0",
        );
        mk(
            "abc123-6.6.12-1.azl4.conf",
            "root=UUID=new selinux=1 enforcing=1",
        );
