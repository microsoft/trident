...
pub fn get_usr_device_path() -> Result<PathBuf, TridentError> {
    let usr_mount_path = if container::is_running_in_container()? {
        ...
        debug!(
            ...
            host_root_path.join("usr").display()
        );
        host_root_path.join("usr")
    } else {
        debug!(
            "Not running inside a container. Using default usr mount path '{}'",
    ...
    let usr_device_path =
        tabfile::get_device_path(Path::new(PROC_MOUNTINFO_PATH), &usr_mount_path)
            .structured(InternalError::GetRootBlockDevicePath)?;
