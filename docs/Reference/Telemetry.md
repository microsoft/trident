...
- `installation_id`: an ID that lets separate events be correlated back to
  the same host installation over time. Normally created (get-or-create,
  never overwritten) at the start of `Trident::install`; the CIH
  update-bootstrap path can also create one, since it adopts a datastore
  without ever calling `install`. Any event that fires before either of
  those has actually run (i.e. before a host's first-ever install has
  created one) instead reports that invocation's own `operation_id` as a
  stand-in `installation_id` -- the same value that will end up persisted
  as the real `installation_id` if that invocation goes on to become the
  first-ever install. A datastore created before `installation_id` was
  introduced (or otherwise adopted without ever going through `install`,
  e.g. offline provisioning) is migrated the first time it is opened: a
