...
`storm-trident run aksagent` is the single supported validation entrypoint for
the annotation-driven `trident-aks-agent` protocol described in the ACL AKS
...
`trident-aks-agent` itself: it deploys a VM, starts fake in-process test
doubles for the Kubernetes API server and the Nebraska/Omaha update server,
seeds bootstrap node labels, and lets the real `trident-aks-agent` binary
...
`tridentd` and real `trident-aks-agent` running inside the VM.

## What It Validates

- `trident-aks-agent` watching its own Kubernetes Node object for
...
Both the enabled/disabled state of `trident-aks-agent.service` and
`/etc/trident/trident-aks-agent.conf` live under `/etc`, which is not part of
...
- **Rust** (latest stable, for building Trident and `trident-aks-agent`)
...
### 1. Build Trident, `trident-aks-agent`, and RPMs
...
```bash
make target/release/trident target/release/trident-aks-agent
...
```bash
# Base image: trident-aks-agent installed but disabled
make artifacts/trident-vm-aks-agent-testimage.qcow2

# Update image: what the agent updates the VM to
make artifacts/trident-vm-aks-agent-update-testimage.cosi
```

:::caution Rebuild after any `trident-aks-agent` change
...
```bash
rm -f artifacts/trident-vm-aks-agent-testimage.qcow2 \
      artifacts/trident-vm-aks-agent-update-testimage.cosi
```
:::
...
```bash
sudo bin/storm-trident run aksagent \
    --artifacts-dir ./artifacts \
    --output-path /tmp/aksagent-output \
...
2. **check-deployment** — Verifies the VM booted and is accessible via SSH;
   writes `/etc/trident/trident-aks-agent.conf` pointing at the
   `localhost:<port>` endpoints storm reverse-SSH-forwards into the VM, then
   runs `systemctl enable --now trident-aks-agent.service`
3. **run-ab-update** — Starts the fake apiserver and fake Nebraska/Omaha
   ...
   update-image label, and waits for `trident-aks-agent` to drive a real
   Trident A/B update to completion (including a real reboot)
4. **collect-logs** — Fetches `trident-aks-agent` and Trident logs from the
   VM via SSH; also runs automatically (with a `journalctl` dump for
   `trident-aks-agent.service`) if `run-ab-update` times out waiting for the
...
This scenario uses a real VM reboot rather than a shim: `trident-aks-agent`
...
If `trident-aks-agent.service` gets stuck reporting `activating` and the test
...
trident-aks-agent.service --no-pager -n 200` on timeout and includes it in
...
The fake Kubernetes API server (`tools/storm/aksagent/proxies/apiserver.go`)
is a minimal, hand-rolled HTTP handler — it does not implement the full
Kubernetes API surface. If `trident-aks-agent` is changed to make a new kind
...
call will simply 404 and (since `trident-aks-agent` treats such client
