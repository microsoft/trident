...
| `TRIDENT_ACL_AGENT_KUBERNETES_CONNECT_MAX_TRIES` | `3` | Total attempts (first attempt included) to connect to the Kubernetes API server before giving up during the startup/recovery Node read only. `0`, `infinite`, or `forever` (case-insensitive) retries forever, mirroring `wget --tries=0`/`--tries=inf`. Does not bound the watch loop, which tolerates transient errors indefinitely (see below). |
| `TRIDENT_ACL_AGENT_KUBERNETES_CONNECT_BACKOFF` | `2s` | Delay between Kubernetes API server connect attempts for the startup/recovery Node read only (a [`humantime`](https://docs.rs/humantime) duration, e.g. `2s`, `500ms`; must be greater than zero). The watch loop's own reconnect delay is governed by `kube::runtime::watcher`'s built-in backoff instead, and it retries indefinitely rather than giving up: a long-lived watch has no reliable "recovered" signal to bound consecutive failures against (a quiet Node produces no stream item on a successful reconnect), so an isolated blip can't safely be told apart from a genuine outage. |
