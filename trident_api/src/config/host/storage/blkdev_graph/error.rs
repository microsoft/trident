...
pub enum BlockDeviceGraphBuildError {
    ...
    #[error(
        "Referrer '{referrer}' [{fs_desc}] references partition(s) \
            of incompatible type '{partition_type}', compatible \
            types are: {compatible_types}. You may be using the wrong \
            target or source type for this referrer."
    )]
    FilesystemIncompatiblePartitionType {
        referrer: BlkDevReferrerKind,
        fs_desc: String,
    ...
    #[error(
        "Filesystem [{fs_desc}] references block device '{target_id}' \
            of incompatible kind '{target_kind}', compatible kinds are: {compatible_kinds}"
    )]
    FilesystemIncompatibleReference {
        fs_desc: String,
        target_id: BlockDeviceId,
        target_kind: BlkDevKind,
        compatible_kinds: BlkDevKindFlag,
    },
    ...
    #[error(
        "Block device '{node_id}' of kind '{kind}' references partitions \
            of different types"
    )]
    ...
    VerityFilesystemInvalidDataPartitionType {
        name: String,
    ...
    #[error(
        "Verity filesystem '{name}' of type '{fs_type}' references block device \
            '{target_id}' of incompatible kind '{target_kind}' as data block, compatible \
            kinds are: {compatible_kinds}"
    )]
    VerityFilesystemIncompatibleReferenceData {
        name: String,
        fs_type: FileSystemType,
        target_id: BlockDeviceId,
        target_kind: BlkDevKind,
    ...
    #[error(
        "Verity filesystem '{name}' of type '{fs_type}' references block device \
            '{target_id}' of incompatible kind '{target_kind}' as hash block, compatible \
            kinds are: {compatible_kinds}"
    )]
    VerityFilesystemIncompatibleReferenceHash {
        name: String,
        fs_type: FileSystemType,
        target_id: BlockDeviceId,
        target_kind: BlkDevKind,
