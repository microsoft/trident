...
/// Get a StorageGraphNode from a VerityDevice reference.
impl From<&VerityDevice> for StorageGraphNode {
    fn from(verity: &VerityDevice) -> Self {
        Self::new_block_device(
            verity.id.clone(),
            HostConfigBlockDevice::VerityDevice(verity.clone()),
        )
    }
}
