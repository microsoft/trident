...
def pytest_addoption(parser):
    parser.addoption(
        '--keep-environment', action='store_true', help='Keep VM environment after tests complete.'
    )

    parser.addoption(
        '--reuse-environment', action='store_true', help='Reuse VM environment from previous tests.'
    )

    parser.addoption(
        '--test-dir', action='store', help='Location to store test files.'
    )

    parser.addoption(
        '--ssh-key', action='store', help='SSH key to use for connecting to VM.', default=os.path.expanduser('~/.ssh/id_rsa.pub')
    )

    parser.addoption(
        '--build-output', action='store', help='Path to the JSON formatted build output.'
    )

    parser.addoption(
        '--force-upload', action='store_true', help='Force upload of tests even if no change was detected.'
    )
...
def deploy_os(test_dir_path: Path, host_config_path: Path, remote_addr_path: Path):
    argus_runcmd([ARGUS_REPO_DIR_PATH / "build" / "netlaunch",
        "-i", "build/installer-dev.iso",
        "-c", "vm-netlaunch.yaml",
        "-t", host_config_path,
        "-l",
        "-r", remote_addr_path])
...
def prepare_hostconfig(test_dir_path: Path, ssh_key_path: Path):
    ...
    with open(TRIDENT_REPO_DIR_PATH / 'functional_tests/trident-setup.yaml', 'r') as file:
...
def deploy_vm(test_dir_path: Path, ssh_key_path: Path, known_hosts_path: Path, create_params = []):
    argus_runcmd(["make", "build/installer-dev.iso"])
    create_vm(create_params)
...
def create_ssh_node(remote_addr_path: Path, ssh_key_path: Path, known_hosts_path: Path):
    with open(remote_addr_path, 'r') as file:
        remote_addr = file.read().strip()
    return SshNode(".", "log", remote_addr, username=TEST_USER, key_path=ssh_key_path, known_hosts_path=known_hosts_path)
    ...
    with ssh_node.ssh_client.open_sftp() as sftp:
        for filename in sftp.listdir('.'):
            if fnmatch.fnmatch(filename, "*.profraw"):
                ssh_node.copy_back(filename, TRIDENT_REPO_DIR_PATH / "target" / "coverage" / "profraw" / filename)
...
def upload_test_binaries(build_output_path: Path, force_upload, ssh_node):
    ...
    for line in open(build_output_path):
        ...
        if "target" in report and "kind" in report["target"] and "lib" in report["target"]["kind"] and "executable" in report and report["executable"]:
...
def vm(request):
    ...
    if not ARGUS_REPO_DIR_PATH.is_dir() or not (ARGUS_REPO_DIR_PATH / "virt-deploy").is_file():
        raise Exception("{} is not a argus-toolkit repo directory".format(ARGUS_REPO_DIR_PATH))
    ...
    if reuse_environment:
        if not test_dir_path or not os.path.isdir(test_dir_path):
            raise Exception("Must specify --test-dir pointing to an existing test directory when using --reuse-environment")
        test_dir_path = Path(test_dir_path)
    else:
    ...
    if reuse_environment:
        ...
        ssh_node = create_ssh_node(test_dir_path / REMOTE_ADDR_FILENAME, ssh_key_path, known_hosts_path)
    else:
        ssh_node = deploy_vm(test_dir_path, ssh_key_path, known_hosts_path, ["-d", "16,16"])
