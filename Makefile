...
# ------------------------------------------------------------------------------
# Third-party OSS attribution notice (NOTICE), generated by cargo-about from
# Cargo.lock. The checked-in NOTICE is regenerated in CI to detect drift, mirroring
# the API schema check above. cargo-about must be installed (see the "Check
...
.PHONY: update-notice
update-notice:
	cargo about $(CARGO_ABOUT_ARGS) -o $(NOTICE_CHECKED_IN)
...
validate-notice:
	...
	@mkdir -p target
	cargo about $(CARGO_ABOUT_ARGS) -o $(NOTICE_GENERATED)
