...
impl OsImage {
    pub(crate) fn cosi(source: &config::OsImage, timeout_in_seconds: u64) -> Result<Self, Error> {
        Ok(Self(OsImageInner::Cosi(Cosi::new(
            source,
            timeout_in_seconds,
        )?)))
    }
    ...
    pub(crate) fn load(
        ...
        let os_image = OsImage::cosi(image_source, timeout_in_seconds).structured(
            InvalidInputError::LoadCosi {
